...
ops = [
    ...
    panel(1, 'API scrape', 'stat', 0, 1, 8, 5, [('up{job="mfd"}', 'Scrape')], '1 means Prometheus reached /metrics. Check individual dependencies below.', 'short', 0),
    panel(2, 'Dependencies ready', 'stat', 8, 1, 8, 5, [('sum(mfd_dependency_ready)', 'of 3')], 'PostgreSQL, NATS and Redis ready probes. Expected value is 3.', 'short', 0),
    panel(3, 'Queued runs', 'stat', 16, 1, 8, 5, [('mfd_runs{status="queued"}', 'Queued')], 'Accepted in PostgreSQL but not yet completed.', 'short', 0),
    panel(4, 'Outbox unpublished', 'stat', 0, 6, 8, 5, [('mfd_outbox_pending', 'Pending')], 'Durable jobs not yet published to NATS.', 'short', 0),
    panel(5, 'Oldest queued', 'stat', 8, 6, 8, 5, [('mfd_oldest_queued_seconds', 'Seconds')], 'Age of oldest queued run. Zero when empty.', 's', 0),
    panel(6, 'Oldest unpublished', 'stat', 16, 6, 8, 5, [('mfd_outbox_oldest_seconds', 'Seconds')], 'Age of oldest unpublished outbox job. Zero when empty.', 's', 0),
    row(101, 'Work moving through the pipeline', 11),
    panel(7, 'Run state', 'timeseries', 0, 12, 12, 8, [('mfd_runs', '{{status}}')], 'Durable run totals, sampled every 15 seconds.', 'short', 0),
    panel(8, 'NATS consumer', 'timeseries', 12, 12, 12, 8, [('mfd_queue_pending_messages', 'Waiting'), ('mfd_queue_ack_pending_messages', 'Awaiting ack'), ('mfd_queue_redelivered_messages', 'Redelivered')], 'Messages in the durable replay consumer. The queue can be empty while runs remain stored.', 'short', 0),
    panel(9, 'Outbox and queued age', 'timeseries', 0, 20, 12, 8, [('mfd_outbox_oldest_seconds', 'Outbox'), ('mfd_oldest_queued_seconds', 'Run')], 'Aging identifies stuck work even when current queue depth looks small.', 's', 1),
    panel(10, 'Dependency probes', 'timeseries', 12, 20, 12, 8, [('mfd_dependency_ready', '{{dependency}}')], '1 means the dependency was ready when /metrics was scraped.', 'short', 0),
    row(102, 'Durability and results', 28),
    panel(11, 'Run and outbox storage', 'timeseries', 0, 29, 8, 7, [('mfd_database_storage_bytes', 'PostgreSQL')], 'PostgreSQL relation and index size for runs and outbox only, not a disk usage metric.', 'bytes', 0),
